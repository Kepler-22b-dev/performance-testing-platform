...
class JMeterRunner:
    ...
    def _inject_error_response_capture(
        ...
        """在 ThreadGroup 的 hashTree 中注入 JSR223 PostProcessor，仅在请求失败时捕获完整请求和响应信息。"""
        ...
        script_text = "\n".join([
            ...
            "        boolean responseDataTruncated = false",
            "        if (responseData.length() > maxBodyChars) {",
            "            responseData = responseData.substring(0, maxBodyChars)",
            "            responseDataTruncated = true",
            "        }",
            "        def samplerData = ''",
            "        try { samplerData = prev.getSamplerData() ?: '' } catch (ignored) {}",
            "        boolean samplerDataTruncated = false",
            "        if (samplerData.length() > maxBodyChars) {",
            "            samplerData = samplerData.substring(0, maxBodyChars)",
            "            samplerDataTruncated = true",
            "        }",
            "        def requestHeaders = ''",
            "        try { requestHeaders = prev.getRequestHeaders() ?: '' } catch (ignored) {}",
            "        def responseHeaders = ''",
            "        try { responseHeaders = prev.getResponseHeaders() ?: '' } catch (ignored) {}",
            "        def requestUrl = ''",
            "        try { requestUrl = prev.getURL()?.toString() ?: '' } catch (ignored) {}",
            "        def requestMethod = ''",
            "        try { requestMethod = prev.getHTTPMethod() ?: '' } catch (ignored) {}",
            "        def data = [",
            "            captureVersion: 2,",
            ...
            "            samplerDataTruncated: samplerDataTruncated,",
            "            responseData: responseData,",
            "            responseDataTruncated: responseDataTruncated,",
            "            truncated: responseDataTruncated",
            "        ]",
